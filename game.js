/* =========================================================================
   ...
   업계 방치형 RPG 장르의 UI 구조·흐름을 폭넓게 참고했고, 콘텐츠·명칭·아트는 결정의 시대 오리지널.
...
/* ★ F3 · IP 세탁 예방 — 이 표의 인명은 전부 결정의 시대 오리지널 조어여야 한다.
   다른 게임 캐릭터명 음차는 물론, 타사 게임 캐릭터의 국문 공식표기와 겹치는 조어도 금지다
   (신규 조어 확정 전 웹 검색으로 타사 IP 충돌 여부를 반드시 확인할 것 — 과거 충돌로 폐기된 이력 있음). */
const HERO_NAMES = {
...
/* ★ v5.126: 영웅 합성 조각 요구량을 독자 곡선으로 재설계했다. ⚠임의수치 — 추후 밸런스 담당이 시뮬로 재조정할 것.
   N→R 진화(=R 해금): 80조각 / R→E 진화(=E 해금): 400조각 / E→L 진화(=L 해금): 1200조각
   N 등급은 소환으로 바로 획득하므로 합성 불가(25 유지 — 소환 가챠용 최소값). */
const HERO_SHARD_NEED = { N:25, R:80, E:400, L:1200 };   // 등급별 합성 요구 조각 (★임의수치, 밸런스 재조정 대상)
...
/* ★ v5.124: 확정 제작(성공 보장 + 시간 스킵) 비용 — 제작서(craftScroll), 등급별.
   종전 구현은 확정 제작이 공짜라 즉시 완성(30 소모·실패 가능)을 완전히 무의미하게 만들었다
   (대표 지적) — 제작 실패 시스템이 장식이 되는 경제 구멍. 자율결정 근거: 즉시 완성 30의 배수로,
   보장 가치가 없는 100% 등급(N·R)은 2배, 실패 확률이 실재하는 E(80%)·L(40%)는 4배·8배로 책정했다. */
...
/* ★ v4.7: 등급별 아이템 수를 재검토한 결과 R·E·L 다수가 부족해 보강했다.
   현재 구성: 무기 N5/R7/E8/L5 · 방어구 N5/R12/E15/L25+ · 장신구 N3/R3/E7/L13 · 특수 N4/R6/E1/L2. */
...
/* 세트효과 9종 — 결정의 시대 오리지널 명칭. 임계값·효과 문구는 스크린샷 판독으로 확정했다.
   ★ A2 정정 — 종전 B3/G-49 의 `half`(2/4 소효과 = 4/4 의 40%) 2단계 설계는 판독 오류였다. 폐기한다.
     세트효과 도감 실측(직접 판독):
     ...
     결론: · 9종 중 8종은 **6세트 단일 임계값**만 존재한다. 2/4 같은 하위 단계는 아예 없다.
           · '작열'(최상위 세트) 1종만 **3 → 6 → 8세트 3단계 누적**이다.
...
/* ★ v5.7: 세트별 구성 아이템 확정 — 자체 설계.
   ...
   **자체 결정**으로 확정한다.
   ...
   설계 원칙
     · 세트 1벌 = 서로 다른 부위 6종(카드 아이콘 6칸과 일치). 작열만 8종(3/6/8 3단계).
...
const COSTUMES = [
  /* 판매 5종 = ★ B7/F1: 실측으로 확인한 '영웅 전용 코스튬 확정구매' 위치(코스튬 탭, 5장·각 3,400루비).
...
/* 티어별 진입 점수(하한). 승급·강등은 이 표와 현재 점수만으로 판정한다(실시간 승강).
   ⚠미확정: 각 티어의 정확한 임계 점수는 아직 확정되지 않았다.
   ...
   ★ N2 — 20260728 추가스샷 9장 재수색: ⓘ 안내 박스가 3줄(주간 리셋 / 데미지 50% 감소 / 입장권 5개)
     ...
     따라서 이 배열은 이번 배치에서도 교체하지 못한다(정확한 임계 점수 확정 시 이 배열만 교체). */
...
/* ★ v5.126: 투기장 보상표 3종(9/13/7행)의 값을 독자 곡선으로 재설계했다. ⚠임의수치 — 추후 밸런스
...
/* ★ N2: 안내문 'ⓘ 매일 입장권 5개가 자동충전 됩니다.' — 날짜 롤오버 시 1회 지급되는 배치 충전량. */
...
/* ★ B7/G-94(§4-5): 회색코인 6품목 상점은 폐지되고 '재료상점'은 루비 결제 MATSHOP 으로,
   ...
   ★ A3-1: 회색코인 획득 경로는 3곳으로 한정한다 —
     ...
     예외로 7일 출석(ATTEND_DAYS 6일차 '회색코인 X20')만 남긴다 — 실측으로 확인된 지급이고,
...
/* ★ B7/G-93: 골드상점 = 골드 결제 5 + 강화석 교환 2(v5.238) + 기록서 골드 라인 1(v5.240) + 루비 환전 2 */
const GOLDSHOP = [
  ...
  /* ★ v5.240: 기록서 골드 구매 라인(자체 설계) — 3200h 시뮬에서 극한 강화 +25 10부위
     완주·각성 44 근접 이후 골드 35.7억이 쌓이기만 했다(싱크 고갈). 기록서는 심화 각성
     31~50의 290권 소모처라, 골드 라인을 주면 엔드게임 최대 골드 싱크가 된다. 가격 3천만은
     전설 망치 X10과 동일(재화 위계 직관적)·시뮬 골드 유입(~190만/h) 기준 약 16h치/권 —
...
const ACCOUNT_PACKS = [
  ...
  /* ★ B7/F1: 개별 영웅 확정구매 카드 2장 — 실측으로 확인한 실제 위치(상점 탭바 밖의 '한정 패키지' 팝업)이며
...
/* ★ v4.7: 몬스터 도감을 실측한 결과 **등급 탭 4개 × 각 30마리 = 120종**으로 보였으나,
   ...
   ★ v5.9 정정 — 재실측 결과: 등급당 ~5종(고유명), "30마리"는 마릿수 선택기였다.
...
/* ★ B5/G-69: 보스 소환 7종 (등급 배분 R×2 / E×3 / L×2).
   설계 — 카드마다 2행×3열(6칸) 요구 재료 그리드가 있고, 일일 횟수 제한 대신
...
/* ★ B5/G-67·G-68 (v4.1 A1-1): 골드던전 5단계 — 실측으로 확정.
...
/* ★ B9/G-117 · F2 재판독 반영: 칭호 29종 (N5 / R9 / E6 / L8 / GM1) — 착용 시 버프.
   ...
   ※ id 는 세이브(S.title·S.titleOwn)와 상점 패키지가 참조하므로 절대 바꾸지 말 것.
     이름·효과·조건은 재판독을 거쳐 확정했다(배열 순서 = 목록 순서 1~29).

   [F2 교정 요지 — 효과 축은 5종으로 한정한다]
     ① 몬스터 골드 획득량 +X%   ② 몬스터 경험치 획득량 +X%
     ③ 제작 시간 -X%            ④ 제작 확률 +X%p            ⑤ 채굴 권한(수치 없는 해금)
     기존 구현의 '드랍률/공격속도/이동속도/모든 스탯' 류는 근거 없는 창작이라 전부 제거했다.
...
// ★ N3: 칭호 목록의 노출 순서 — GM 1종이 영웅(E)과 레전더리(L) 사이에 끼어 있다(칭호창 13장 실측)
const TITLE_GORDER = ['N','R','E','GM','L'];
/* ★ F2: 월드챗 문구 칭호 4종 — '특정 문구를 월드챗에 전송'하면 즉시 지급된다.
   문구는 결정의 시대 오리지널. 전송 판정은 chatTitleCheck() 가 한다. */
...
const TITLES = [
  ...
  /* ⚠비전미확인 — 촬영대기 (N3 재판독에서도 미해결).
     칭호창 13장 전수 판독 결과 조건 원문이 여전히 '초급자가 얻을 수 있는 호칭'이라는 순환 서술이라
  ...
  /* 실측 확정 조건 = '일반 등급 영웅 **10명** 보유'(N3 칭호창 실측).
  ...
  // ★ N3: 조건문에 '(해골 장비 제외)' 부기가 붙어 있다(숙달된 손·행운의 인장·축복받은 손 3종 공통) → 문구·집계 양쪽 반영. isSkullGear() 참조
  { id:'skilled',    g:'R',  n:'숙달된 손',    fx:'제작 확률 +2%p',       e:{crate:0.02},
    cond:'영웅 등급 이상 장비 제작 5연속 성공 (해골 장비 제외)',  have:()=>titleStat('craftWinBest')>=5 },
  // 최상위 보스 던전 클리어 조건 → 결정의 시대의 레전더리 보스 소환(몽마·수호거상) 클리어로 구현
  ...
  /* ★ N1: 조건 '주사위 누적 1,000개 소모'를 그대로 복원.
  ...
  /* ---- GM (1) ----
     ★ N3: 칭호 목록에서 이 카드는 **영웅(E)과 레전더리(L) 사이**에 끼어 있다(스크롤 13장 실측).
     등급 색상만으로는 경계가 애매했으나 등급별 개수(N5+R9+E6+GM1+L8=29) 검산으로 GM 칸임이 확정됐다.
     종전에는 목록 맨 끝(29번)에 두었는데 이는 실측 순서와 어긋나므로 여기로 옮겼다(TITLE_GORDER 도 함께 정정). */
  ...
  /* ★ A2 정정 — 종전 '코스튬 8종 보유' 매핑은 오답이었다(조건을 세트가 아닌 컬렉션으로 잘못 읽음).
     ...
     · 소환 마릿수 증가폭(%)은 확정된 수치가 없다 — 수치 미표기, 자체 결정 필요.
...
/* ★ A2: `own:true` 칭호(현재 '불씨의 벗' 1종)는 착용하지 않아도 **보유만으로** 플래그 효과가 켜진다.
   조건 원문이 "보유 시 … 효과 자동 적용"이라 다른 칭호의 착용 방식과 명시적으로 구분된다.
   수치형 효과(titleEff)는 종전대로 착용분만 적용 — 보유만으로 적용되는 것은 이 소환수 플래그뿐이다. */
...
// '불씨의 벗' — 동시 등장 몬스터 +2 (보유만으로 적용). 증가폭은 확정된 수치가 없어 자체 결정.
...
const NOTICES = [
  ...
  { cat:'[업데이트]', ic:'⚔️', t:'대장간이 더 뜨거워졌습니다 — 방치 정산 12시간 · 전투 연출 개편', d:'2026-09-25',
    body:'군주들에게 알립니다.<br><br>'+
      '· <b>🌙 방치 정산 최대 12시간</b> — 자리를 비운 사이 쌓이는 골드의 한도가 8시간에서 <b>12시간</b>으로 늘었습니다. 푹 자고 돌아와도 손해가 없습니다.<br>'+
      '· <b>⚔ 전투력 표시</b> — 전장 왼쪽 위에 전투력이 늘 보이며, 강해질 때마다 <b style="color:#7fe08a">+N ▲</b> 로 알려 드립니다. 누르면 전투력 구성을 볼 수 있습니다.<br>'+
      '· <b>🛡️ 전멸 분석</b> — 사냥에서 전멸하면 무엇이 부족했는지, 무엇을 하면 되는지를 수치로 안내하고 안전한 사냥터로 바로 옮겨 드립니다.<br>'+
      '· <b>🔨 강화 결과</b> — 강화할 때 능력치가 얼마나 올랐는지(전 → 후)를 바로 보여 드립니다.<br>'+
      '· <b>🔥 전투 연출</b> — 더 부드러운 움직임, 읽기 쉬운 피해 숫자(치명타·스킬 구분), 쓰러지는 연출, 우두머리 등장 경고가 더해졌습니다.<br>'+
      '· <b>✨ 연출 설정</b> — 흔들림·번쩍임·피해 숫자는 [설정 → 연출]에서 각각 끌 수 있습니다.<br>'+
      '· <b>🔒 진행도 보호</b> — 같은 기기에서 게임을 두 창으로 열면, 나중에 연 창만 저장해 진행도가 서로 덮어쓰이지 않게 했습니다.' },
  /* ★ v5.303: 신규 콘텐츠 2종(잔불 v5.294·용광로 v5.300) 통합 안내 — 축제만 공지돼 있고
     나머지는 모험 팝업 발견에만 의존했다. 공지 추가로 미열람 점이 자동 점등(NOTICES.length
     vs noticeSeen 관례) — 기존 이용자에게 새 던전을 능동적으로 알린다. */
  { cat:'[업데이트]', ic:'🕯️', t:'새로운 시련이 열립니다 — 잔불의 미궁과 용광로 시련', d:'2026-09-18',
    body:'군주들에게 알립니다.<br><br>대장간 깊은 곳에서 두 시련이 눈을 떴습니다. 모두 <b>[모험]</b> 팝업에서 입장할 수 있습니다.<br><br>'+
      '· <b>🕯️ 잔불의 미궁</b> — 일 1회 무료 진입. 문마다 위험이 다릅니다(적 전투력 8천 / 1만6천 / 2만6천). '+
      '실패하면 보상 없이 오늘의 기회만 끝나고, 클리어하면 강화석과 골드를 얻습니다. 최고 문 기록이 남습니다.<br>'+
      '· <b>⚒️ 용광로 시련</b> — 매월 1회 도전하는 거대한 수호자(적 전투력 3만·장기전). '+
      '패배하면 다음 달까지 재도전할 수 없습니다. 클리어 보상은 강화석 X300 + 골드 1,200만.<br><br>'+
      '축제(v5.295)와 함께 — 매주 월요일 교체되는 주간 축제는 전장 왼쪽 위 배지에서 확인하세요.' },
...
function load(){
  ...
  S = freshState();
  S.lastSeen = Date.now();
  // ★ v5.83: 설계 기준 — 시작 보유는 일반(N) 영웅만.
  //   R 등급은 합성(진화)으로 획득해야 함. 종전엔 HERO_006(R)을 지급해 이 기준과 어긋났다.
...
/* ★ 2026-09-25: 오프라인 상한(시간) 단일 출처 — 종전엔 8 이 game.js 4곳(부팅 정산·복귀 정산·정산 화면 2곳)과 시뮬·스모크에
   ...
   ★ 8 → 12 (v5.322): 8시간 상한은 수면 시간(7~9h)과 겹쳐 '아침마다 상한에 걸려 손해 본 느낌' 을 준다
   (자료: 강의 규칙집 090강 — 12시간이 초기 설계 기준으로 가장 적합, 상한은 복귀 빈도와 교환 관계).
   실측(캐주얼 2400h·시드 42, 8h vs 12h): 등급 도달 R24.5/E97/L169h 완전 동일(초반 곡선 불변) · 최종 CP +1.8%
   · 액션 +17.5% · 후반(600h~) 행동 밀도 3.3→4.5/접속일 · 최장 공백 7.5h 동일 · 늘어난 골드는 결정 가호 싱크가 흡수.
   상한은 올리기만 한다(설계 종합 U5 — 캡 단조) — 되돌려 낮추면 기존 이용자 약속 위반이다. */
...
/* ★ B6/G-81→v5.84: 투기장 전용 3인 편성 (3v3 기준).
...
const Battle = (()=>{
  ...
  function layoutHeroes(keep){
    ...
    heroes = src.map((h,i)=>{
      ...
      return {
        ...
        /* ★ v5.216 시도 후 되돌림: 스킬 등급 해금(N=1차만, R=1~2차…)은 '선언뿐'이었다는
           감사(정합 12호)가 맞지만, 해금 로직을 어떤 방식으로 넣어도(선택 루프 범위 축소 ↔
           잠긴 스킬 쿨 Infinity) D1 자기일관성이 깨진다(실측: rngDrawCount 524↔522, dmg·kills
           동일 — 스킬 사용 시점이 달라지면 프레임당 누산기·몹 상태가 미세하게 갈라져
           비시드 난수 소비 타이밍이 어긋나는 것으로 추정). 결정론(전투 재현성)이 스킬 해금
           표현보다 우선이므로 유보한다 — 정합 축에서는 '전 직업이 전 스킬 사용 가능'으로
           동작함을 안내문에 반영하는 게 맞다(아래 itemFlavor 참조). */
        skillCD:[0,0,0,0],
  ...
  function spawnMob(boss){
    const t=tierDef();
    /* ★ 홈 1인 서바이벌: 몹이 사방에서 스폰되어 중앙 영웅을 향해 직진 (실측: 10~20마리가 영웅을 원형으로 둘러쌈).
  ...
  function update(dt){
    ...
    if(mode==='dungeon' && dg){
      ...
      // ★ v5.9: 마릿수 선택기(S.mobCount, 기본 30)가 홈 필드 동시 스폰 상한을 결정한다.
  ...
  function drawHero(h){
    ...
    /* ★ 홈 1인: 머리 위 정보 (이름표 + HP/MP 2단 바). 던전 파티는 종전대로 직업명+HP 1단.
       실측(화면 흐름 판독 라인 33/73): "영웅 머리 위에 이름 + 원형 레벨 뱃지 + 적색 HP바/청색 MP바 2단". */
    if(isHuntSolo()){
      const nm = h.name || h.job.name;
      /* ★ v5.54: 실측 기준 레이아웃 — 이름 + 원형 레벨 뱃지(이름 좌측) + HP/MP 바.
  ...
  // ★ 홈(1인 서바이벌)에서는 기여도 패널을 숨긴다 — 실측: 홈에는 기여도·웨이브·제한시간 표시 없음.
  let contribT=0;
...
const GUILD_MEMBERS = [
  /* ★ 2026-09-11: '불철' 을 master → officer 로 정정.
     길드를 창설하면 항상 그 플레이어 자신이 길드장이 된다(guildCreate 가 S.guildRank='master' 로
     못박는다) — 그런데 이 더미 명단의 첫 줄도 role:'master' 였다. 실제로 길드를 만들어 보니
     길드원 목록에 "길드장" 배지가 두 개(불철 · 군주) 동시에 떴다. 한 길드에 길드장은 한 명이다.
     '불철' 은 이 게임 전역에서 재사용되는 얼굴마담 NPC(CHAT_NAMES·WB_RANK 등)라 이름은 두되,
     역할만 임원으로 내린다. */
  ['불철',       'officer', 184200,  0, '07-27', 42],
...
function enterGuildRaid(auto){
  ...
  if(auto) go();
    ...
    // ★ v4.8: 길드 레이드 확인창은 [예]좌 / [아니요]우 로 설계한다(실측 08_10_00_330).
...
const TUT = [
  ...
  /* ★ v5.112: STEP7 은 '영웅탭 → 합성'(조각→등급업→스킬 해금)이다. 종전의 '용광로 재료 합성'은
     이 흐름에 없고, 정작 코어 성장축인 조각 합성을 튜토리얼이 한 번도 안 가르치고 있었다.
  ...
  /* ★ v5.112: STEP8 은 '진영 편성 저장 + 투기장 입장' 두 가지다(0/2). 편성만 저장하고 끝나면
...
/* ★ v5.1: 하위 화면을 '부모 모달 위 오버레이'로 띄운다.
   설계상 재료합성·망치제작·제작결과·아이템상세·장비강화·주사위·영웅상세를 전부 부모 화면 위에
...
const MODALS = {
  ...
  optionReroll:{ title:'옵션 재설정', render(b, heroId){
    ...
    REROLL_MILES.forEach(m=>{
      const t=el('div','rr-ms'+(S.rerollSpent>=m.at?' on':''),`${m.at}개 / ${GRADES[m.g].name}`);   // 표기는 자릿수 구분 없음
    ...
    REROLL_ROWS.forEach((r,i)=>{
      ...
      lk.onclick=()=>{
        ...
        if(S.rerollLock[i]) toast('잠금되었습니다.');   // 해제 토스트는 확정 근거 없음 → 라벨 전환만
      ...
      else if(r.g==='L') val.innerHTML='<span class="rr-art">⚔</span>';   // 레전더리 빈 칸 = 교차검 장식
      fr.appendChild(val); row.appendChild(fr);
      // ↻ 재설정
      // ⚠비전미확인 — 촬영대기: 사선(비활성) 아이콘의 조건이 무엇인지 아직 모른다.
      //   "주사위 부족"이라는 우리 가설은 스크린샷이 직접 반증한다 —
  ...
  /* ---------- 투기장 (★ B6: G-81~G-92) ----------
     설계: 화면 하나 안에서 4탭(랭킹 / 순위 주사위 / 순위 골드버프 / 티어 골드)만 갈아끼운다.
  ...
  arena:{ title:'투기장', render(b){
    /* ★ N2: 탭 라벨은 짧은 2~4자('보상'/'버프'/'매일 보상')로 둔다. 키(dice/gbuff/tier)는 그대로 두고
    ...
    let infoOn = false;   // ⓘ 안내 박스(좌측 패널 안에서 리스트와 교체되는 인패널 박스)
    ...
    /* ★ N2: 표 제목 — 버프/매일 보상 표 상단에만 `-제목-` 한 줄이 붙는다(보상 표에는 없음). */
    ...
    function paintBox(){
      rankBox.innerHTML='';
      /* ★ N2: 헤더('랭킹' 라벨 + ⓘ)는 탭과 무관하게 항상 좌측 패널 상단에 남는다. */
      ...
      if(infoOn){
        /* ★ N2: ⓘ 안내 박스는 다이얼로그가 아니라 리스트 자리를 덮는 인패널 박스이며,
      ...
      if(tab==='rank'){
        ...
        /* ★ N2: 종전 각주('시즌 종료 시 1회 지급')는 표제 '매일 티어 골드 보상'과 정면으로 어긋나
    ...
    // ★ N2: 안내문 확인 사실(데미지 50% 감소 · 매일 입장권 5개 자동충전)을 하단 힌트에도 반영
  ...
  /* ---------- 상점 (★ B7: G-93~G-101, G-103) ----------
     설계: 하단 고정 9카테고리 · 4열 아이콘 탭(4+4+1 3행)
  ...
  shop:{ title:'상점', render(b){
    ...
    /* ★ v5.0: 상점의 지배적 위젯은 '리본 헤더 + 좌(아이콘 위·구매버튼 아래) / 우(품명·수량·가격)'
    ...
    function render(){ body.innerHTML=''; const now=Date.now();
      ...
      /* ── ③ 영웅 (G-102 결정): 개별 영웅 조각 확정 구매 ──
         ...
         카드 구성(재판독으로 확정한 cardLayout): ① 컬러 리본 헤더 '[영웅명] 조각 X n'
      ...
      /* ── ⑦ 루비 (G-99): 4단계 + 프로모 4행 ── */
        ...
        // ★ F2: 루비 상자(충전 상품) 구매는 칭호 '귀빈'의 획득 트리거다(과금 트리거 위치를 그대로 따름).
  ...
  inventory:{ title:'인벤토리', render(b){
    ...
    /* 기타 재화.
       ★ v4.7.1: '재료풀 N/R/E/L' 4칸을 제거했다. 두 가지 이유가 겹친다 —
         ...
         ② 실측 결과 이 4칸은 애초에 존재하지 않는 구성이었다(04_장비_인벤토리 실측).
  ...
  /* ---------- 마을 ---------- */
  /* ★ v5.3: 마을 화면은 세로 리스트가 아니라 **좌우 2열 카드**로 설계한다 —
  ...
  village:{ title:'마을', render(b){
    ...
    b.appendChild(g);
    // 하단: 두 재료를 각각 표시(서로 다른 두 수치를 따로 노출)
        ...
        styledConfirm('소환 하시겠습니까?', ()=>{
          ...
          enterDungeonFight({ name:bt.n, col:G.color, foeCP:bt.foe, kind:'boss', dur:30,
            ...
            reward:()=>{ if(bt.drop) bt.drop();
              // ★ F2 칭호 '중견 대장장이' — '최상위 보스 던전 클리어' 조건을 레전더리 보스 처치로 구현
  ...
  /* ★ B5/G-70·G-71·G-72·G-66: 월드보스
       ...
       ★ N2 — 20260728 재수색 2장, 전 스크롤 구간: 월드보스 화면에는 순위별 차등 보상표가 존재하지
            않는다는 것이 확정됐다(랭킹 행 = 순위·닉네임·점수 3열뿐, 보상 컬럼 없음 / 하단은 '전투 보상 X2'
            고정 1줄). '순위→보상 환산식 미확인'은 '해당 없음 확정'으로 종결 → 코드 수정 없음. */
  ...
  /* ★ B9/G-123·G-124·G-125: 7일 출석 — 1일 1회 날짜 검증 + 7열 그리드 + 수량비 40:40:150:10:50:20:250 */
  ...
  help:{ title:'도움말', render(b){
    const topics={
      ...
      /* ★ v5.198: '소환속도 1.5초' 문구 정정 — 코드에 그런 스탯이 없다(실제 스폰 간격은
         ...
         찾으러 다닌다 — 도움말은 실재하는 조절 수단(마릿수 선택기·프리셋 v5.191)을 알려준다. */
      '몬스터':'몬스터를 소환해 사냥하면 등급별 재료를 얻습니다.<br><br>몬스터는 약 0.4~1초 간격으로 계속 소환되며, 동시에 몰리는 마릿수는 몬스터 화면의 마릿수 선택기(최소 5 · 중간 15 · 최대 30)로 조절합니다. 5웨이브마다 체력 6배의 군주(보스)가 등장해 처치 시 등급 재료를 대량 드랍합니다.',
      /* ★ v5.168: 수치를 CRAFT 테이블에서 동적으로 생성 — 종전 문구(희귀85·영웅55·레전더리40%)는
         실제(N100·R100·E80·L40%)와 어긋났고, '실패 시 100% 환급'도 v5.124 경제 봉합(90% 환급)과
         어긋났다. 표기가 정본(코드)에서 파생되면 다시 어긋날 수 없다. [다시 제작](v5.151)도 안내. */
      '제작':`대장간에서 등급×부위로 장비를 제작합니다. 기본 확률 ${GORDER.map(g=>`${GRADES[g].name} ${Math.round(CRAFT[g].p0*100)}%`).join(' · ')} — 칭호·버프로 오를 수 있고 길잡이 구간은 100%로 고정됩니다. 실패 시 재료 90% 환급, 결과 창의 [다시 제작]으로 바로 재도전할 수 있습니다.`,
      '인벤토리':'재료·완성 장비·소환권·재화를 보관합니다.<br><br>재료: 일반·희귀 최대 2000개, 영웅·레전더리 최대 900개(초과분 미획득)<br>무기: 종류당 최대 99개',
  ...
  /* ★ v5.229: '장기 목표'의 강화 기대 비용 — 종전 3.2억(v5.229 시뮬 '실측')은 표본 편향이었다: +20 에 도달하지 못한 부위까지 평균에 넣어
     시도 수(약 26회)가 절반으로 잡혔다. v5.373 재판정(교정 시뮬 + 해석해 + 몬테카를로 20만 회): +10→+20 부위당 평균 4.76억(시도 약 36.6회 · 전설 망치 81.5개 포함) ·
     중앙 4.0억 · 하위 10% 1.7억 · 상위 10% 8.7억. 10부위 약 47억. (시뮬 교차 확인: +25 완주 부위당 시도 50.4회 = 해석해 52.0)
     (그 전 몬테카를로 4.8억이 '실패마다 망치 소모'로 틀렸다던 v5.229 의 결론도 결과적으로 거의 같은 값 — 망치는 실패 중 파괴 분기에서만 소모되는 게 맞다.) */
  strategy:{ title:'공략', render(b){
    ...
    3) 몬스터 도감의 재료 참고로 부족한 재료를 직접 사냥한다 (등급 완성 시 보상)<br>
    4) 세트 6세트를 노린다 — 착용창의 세트 칩이 다음 조각을 알려준다<br>
    5) 희귀에서 영웅(약 2일)·레전더리(약 6일) 장비로 전 부위를 바꾼다 <span class="mut">(하루 8시간 접속 기준 · 첫 레전더리 약 3일)</span><br><br>
    ...
    · 강화 +11~25: 망치로 파괴를 막으며 도전 — +20까지 부위당 평균 약 4.7억 골드(전설 망치 약 80개 포함 · 운에 따라 2억~9억), +21~25 극한(성공 30%)<br>
    ...
    function render(){ body.innerHTML='';
      if(tab==='임무목록'){
        ...
        /* ★ v5.219: 업적 축 4종 추가(영웅 합성·각성·분해·레전더리 제작) — v5.172 개편 당시
           ...
           첫 성공 — v5.373 재판정: 풀 약 29.5h · 캐주얼 약 75.5h(약 3일). 종전 '≈ 6일'은 잘린 시뮬 값). 보상 없음(종전 원칙). */
  ...
  guild:{ title:'길드', render(b){
    ...
    function drawFoot(){
      ...
      const items = guildJoined()
        /* ★ v5.2: 길드 메인 헤더(정원·길드명·공지·누적점수 석판)와 멤버 목록이 뒤에 남은 채
        ...
        enterDungeonFight({ name:`약탈 · ${target}의 영지`, col:'#c8752e',
          ...
          reward:()=>{
            ...
            /* ★ A3-1: 회색코인 획득 3경로 중 '약탈'. ⚠비전미확인 — 촬영대기: 약탈 1회당 회색코인 수량은
               결과 팝업 판독으로는 확인되지 않았다(효과 안내 3줄에도 회색코인 항목 없음). 길드 레이드
  ...
  /* ★ v5.3: 코스튬 화면은 '목록 ↔ 상세' 화면 전환이 아니라 **한 화면 마스터-디테일**로 설계한다.
  ...
  monster:{ title:'몬스터', render(b){
    ...
    b.appendChild(tabrow);
    /* ★ v5.9: 마릿수 선택기 — '30마리' 상하 화살표(실측 판독 라인 121, UI카탈로그 110).
  ...
  /* ★ B9/G-135(§4-9)·G-136 → N3/§7-9 갱신: power = '절전 시작' 전용(배터리 + 안내문 + 시작 버튼).
     [실측 반영] 절전 화면 8장을 확보해 종전의 `⚠비전불확실`(캡처 0장)을 해제했다 — 오버레이 구성·해제 방식은
       startPowerSave() 주석 참조. 정규 진입은 **좌상단 명패 탭(확인 없이 즉시)** 이며,
  ...
  titles:{ title:'칭호', render(b){
    ...
    function draw(){
      ...
      if(view==='보유'){
        ...
        /* ★ N3: 보유 카드는 [칭호명 | 효과박스 | 착용/착용중] 아래에 **접었다 펴는 '조건' 서브텍스트**가 붙는다. */
        ...
        // ★ N3: 정해진 목록 순서대로 — 일반 → 희귀 → 영웅 → GM → 레전더리
...
/* ★ N3/§7-10: 외부 서비스 링크 정본.
   url 은 **의도적으로 비어 있다.** 참고 자료에 찍힌 실제 주소·서비스 브랜드는 타사 자산이라 코드에 옮기지 않는다.
...
/* 12칸 자원 그리드 정본 — settle(정산 상세)과 공유한다.
   ⚠미확정: 참고 그리드의 6칸은 아이콘만 있고 라벨이 없어 재화를 특정하지 못했다(확인 대기). */
function pwCells(){
  return [
...
/* ------- 영웅 상세 -------
   ...
   ★ B4/G-54 (v4.2 A3-2): [강화] 탭 = 영웅별 강화. 재수색으로 확정된 규칙만 반영한다.
...
/* ★ A3-2: 강화 1회당 조각 소모량 — 패널 적색 문구로 확정된 300 고정.
...
function itemDetail(e, heroId){ _itemDetailHeroId=heroId||null;
  ...
  // ★ v5.126: 장착 경고문을 재작성했다. 규칙(장착 시 같은 부위 기존 장비 파괴)은 유지.
...
/* ------- 제작 결과 ------- */
/* ★ N3: 제작 성공 스트릭 칭호(숙달된 손·행운의 인장·축복받은 손)의 조건 부기 '(해골 장비 제외)' 실집계용 판정.
...
function resolveCraft(forceSuccess){
  ...
  /* ★ F2 → N3 갱신: 제작 계열 칭호는 누적이 아니라 '연속(스트릭)' 조건이다.
     ...
     · '영웅 등급 이상' 장비 제작 성공 스트릭 5/10/15 → 제작 확률 칭호
       ★ N3 실측: 조건문 3종(숙달된 손·행운의 인장·축복받은 손) 모두 끝에 **'(해골 장비 제외)'** 부기가 붙는다.
...
/* ------- 투기장 전투 (기여도% 포함) ------- */
// ★ 투기장: 콜로세움에서 상대 소환수 4마리와 실전 전투 → WIN/Loss(변경 전→후) → 3초 후 자동 연전
/* ★ v5.5: 투기장 매치 제한시간은 2:00 이다(실측 159행, 01:58→01:49 직접 관찰).
...
/* ★ B6/G-88·G-90: 랭킹 리스트 행 생성. 내 순위가 5위 이내로 올라와도 NPC 와 순위가 겹치지 않게 민다.
   ★ N2/G-93: 돋보기(유저 정보) 팝업이 요구하는 필드(길드·서버·ID)를 행에 같이 실어 보낸다.
     기존 시드 ID 는 ISO 타임스탬프 문자열(예: 2026-05-02T23:40:25.551Z)이라 계정 생성 시각으로 읽힌다 →
...
/* ★ N2: '랭킹은 매주 월요일 12시에 초기화 됩니다.'(안내문) — 주간 리셋 스케줄러.
...
function arenaFight(){
  ...
  /* ★ N2: 매칭 기준 전투력을 '보유 전체(totalCP)'에서 '실제 출전하는 PVP 4인'으로 바꾼다.
     기존 설계는 상대의 4인 편성과 겨루는데, 종전 기준은 보유 영웅이 늘수록 상대만 강해져
  ...
  /* ★ N2: dmgMul — 안내문 '투기장에선 모든 데미지가 50% 감소 됩니다.' */
...
function arenaResult(win, foeName, foeCP, foeTier){
  ...
  /* ★ v4.5 (대표 결정): 결과창 배너를 '연승/연패 스트릭'으로 한다.
     ...
     ※ 종전 구현은 이렇게 동작하지 않았다 — 결과창 배너를 5배 확대해 판독한 결과
  ...
  /* ★ v4.4: "싸웠고 뭘 받았다"가 한눈에 오도록 획득량을 명시한다.
     기존 결과창은 before→after 숫자만 주고 획득량을 안 알려줘서 역산해야 했다(대표 지적).
     before→after 2열 구조는 그대로 두고 델타 한 줄만 추가한다. */
...
/* ------- 던전 실행 ------- */
// ★ 던전 입장 플로우: 입장 → 홈 캔버스가 던전 전투로 전환(몬스터 여러 마리 or 보스 1마리와 실전)
...
/* ★ B5/G-72 (v4.1 A1-2): 월드보스 보상 = 순위·점수 무관 '전투 보상 X2' 고정.
   재수색 결과 UI에는 순위 구간별 차등 지급표가 존재하지 않는다(두 세션 결과 팝업 모두
...
/* ⚠미확정: 화면은 'X2' 배수만 노출하고 배수의 기준이 되는 '전투 보상' 원본 수량이
   결과 팝업에서 판독되지 않았다. 아래 기준값은 종전 참가(11위 이하) 구간 수치를 그대로 승계한
   임시값이며, 근거 자료 확보 시 교체할 것. 점수→보상 환산 공식은 확정되지 않아 만들지 않는다. */
...
/* 전투 결과 점수 — 결과 팝업은 성패 대신 정수 'N점'을 띄우고 별도로 적립한다(실측 판독 #44).
...
function wire(){
  ...
  /* ★ N3/§7-9: 좌상단 명패 탭 = 절전 즉시 진입(확인 모달 없음). 실측 — 절전화면 캡처 5장의 대표 주석 근거. */
