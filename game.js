...
/* ★ v5.63: 장비 슬롯명 → 아이콘 이모지 매핑 (인벤토리/착용창/제조결과 공용). */
const EQUIP_ICONS = [
  ['단검','🗡️'],['대검','⚔️'],['도끼','🪓'],['지팡이','🪄'],['장궁','🏹'],['강궁','🏹'],
  ['쌍검','🗡️'],['전곤','🔨'],['사슬낫','⛓️'],['창','🔱'],['낫','🪒'],['완드','🪄'],['소드','⚔️'],['검','🗡️'],
  ['방패','🛡️'],['투구','⛑️'],['상의','🥼'],['하의','👖'],['신발','🥾'],
  ['견갑','🎽'],['각반','🦵'],['완갑','🧤'],['망토','🧣'],['벨트','🎗️'],
  ['면갑','😷'],['흉갑','🦺'],['정강이받이','🦿'],['손목보호대','🧵'],['어깨받이','🎽'],
  ['반지','💍'],['목걸이','📿'],['귀걸이','🧿'],['팔찌','⌚'],['인장','🔖'],['귀고리','🪬'],['부적','🧿'],
  ['고서','📖'],['정수','🔮'],['물약','🧪'],['곡괭이','⛏️'],['나침반','🧭'],['소환 부적','🎴'],
];
...
const MODALS = {
  ...
  equip:{ title:'장비', render(b, heroId){
    ...
    S.equips.slice(0,15).forEach(e=>{ const c=el('div','cell gframe grade-'+e.grade); c.style.setProperty('--gc',GRADES[e.grade].color);
      c.innerHTML=`<div class="ei" style="font-size:22px">${eImg(equipIcon(e.slot),2)}</div><div class="cn">${e.slot}</div>${e.enh?`<div class="lvl">+${e.enh}</div>`:''}`; c.onclick=()=>itemDetail(e, cur.hero_id); tray.appendChild(c); });
    b.appendChild(tray);
  }},
...
function itemDetail(e, heroId){ _itemDetailHeroId=heroId||null;
  ...
  card.innerHTML=`<div class="ic-head"><div class="ic-ico grade-${e.grade}" style="--gc:${G.color}">${eImg(equipIcon(e.slot),2)}</div>
...
function openEnhance(e){
  ...
  b.appendChild(el('div','center',`<div class="ei" style="font-size:52px">${eImg(equipIcon(e.slot),2)}</div><div class="big" style="color:${GRADES[e.grade].color}">${GRADES[e.grade].name} ${e.slot} +${e.enh}</div>`));
